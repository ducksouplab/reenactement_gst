...
def run_shake_demo(source_image, output_file):
    # Pipeline components
    pipeline_str = (
    ...
    print(f"Running shake demo for {source_image} -> {output_file}")
    ...
    amplitude = 0.3  # 0.3 radians ~ 17 degrees
    frequency = 1.0  # 1 shake per second

    def update_pose():
        elapsed = time.time() - start_time
        # Sine wave for head shake (yaw)
        yaw_offset = amplitude * math.sin(2 * math.pi * frequency * elapsed)
        lp.set_property("pose-yaw-offset", yaw_offset)
...
if __name__ == "__main__":
    ...
    run_shake_demo(args.source, args.output)
